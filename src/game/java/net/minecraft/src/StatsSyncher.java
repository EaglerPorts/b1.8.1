...
public class StatsSyncher {
	...
	private VFile2 unsentDataFile;
	private VFile2 dataFile;
	private VFile2 unsentTempFile;
	private VFile2 tempFile;
	private VFile2 unsentOldFile;
	private VFile2 oldFile;
	...
	public StatsSyncher(Session var1, StatFileWriter var2, VFile2 var3) {
		this.unsentDataFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + "_unsent.dat");
		this.dataFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + ".dat");
		this.unsentOldFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + "_unsent.old");
		this.oldFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + ".old");
		this.unsentTempFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + "_unsent.tmp");
		this.tempFile = new VFile2(var3, "stats_" + var1.username.toLowerCase() + ".tmp");
	...
	private Map func_27408_a(VFile2 var1) {
		...
		try {
			var2 = new BufferedReader(new InputStreamReader(var1.getInputStream()));
			String var3 = "";
	...
	private void func_27410_a(Map var1, VFile2 var2, VFile2 var3, VFile2 var4) throws IOException {
		PrintWriter var5 = new PrintWriter(new OutputStreamWriter(var3.getOutputStream()));
	...
	public void beginReceiveStats() {
		if(this.isBusy) {
			...
			this.isBusy = true;
			// B1.8 TODO: (new ThreadStatSyncherReceive(this)).start();
		}
	}
	...
	public void beginSendStats(Map var1) {
		if(this.isBusy) {
			...
			this.field_27427_l = 100;
			this.isBusy = true;
			// B1.8 TODO: (new ThreadStatSyncherSend(this, var1)).start();
		}
	}
	...
	static VFile2 getUnsentDataFile(StatsSyncher var0) {
		return var0.unsentDataFile;
	}

	static VFile2 getUnsentTempFile(StatsSyncher var0) {
		return var0.unsentTempFile;
	}

	static VFile2 getUnsentOldFile(StatsSyncher var0) {
