...
import net.lax1dude.eaglercraft.sp.server.export.RandomAccessMemoryFile;
...
public class RegionFile {
	...
	public RegionFile(VFile2 var1) {
		...
		try {
			byte[] var0 = (var1.length() > 0) ? var1.getAllBytes() : new byte[0];
			this.dataFile = new RandomAccessMemoryFile(var0, (int) var0.length);
			if(var1.length() == 0) var1.setAllBytes(var0);
			int var2;
			...
			if((this.dataFile.getLength() & 4095L) != 0L) {
				for(int i = 0; i < (int)((4096 - (this.dataFile.getLength() & 4095L)) & 4095L); i++) {
			...
			try {
				...
				if(var3 == 0) {
					...
					if(var4 + var5 > this.sectorFree.size()) {
						...
						if(var6 > 4096 * var5 || var6 <= 0 || var6 > 4096 * var5) {
	...
	public void close() throws IOException {
		fileName.setAllBytes(dataFile.getByteArray());
	}
}
